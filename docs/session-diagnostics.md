...
Key history and its two recent-key caches each have a 2 MiB serialized-byte budget and a 1,000-record limit. This reserves at most 6 MiB for the three stores in each storage area. Private history uses the same limits in session storage. Writes sanitize manifest metadata and evict the oldest captures first, keeping records with the same capture ID together. PSSH values are never truncated. Existing oversized history is bounded on its next write.

If the newest capture cannot fit intact, the write is rejected and previously saved records remain. A browser storage rejection is also reported as a key-history save failure in the local failure alert and badge. The history stage is marked failed in the trace, while extracted keys are still returned to the page. History and recent caches are separate writes, so a cache failure can leave the capture saved in history. If diagnostics storage also fails, the extension attempts to display the error directly in the badge tooltip and logs it to the background console.
