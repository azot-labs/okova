...
test('device convenience access follows the asynchronous EME contract', async () => {
  ...
  const pending: Promise<MediaKeySystemAccess> = credentials.requestMediaKeySystemAccess(
    'com.widevine.alpha',
    [{ initDataTypes: ['cenc'] }],
  );
  expect(pending).toBeInstanceOf(Promise);
  const access = await pending;
  expect(access.getConfiguration().sessionTypes).toEqual(['temporary']);
  await expect(
    credentials.requestMediaKeySystemAccess('com.widevine.alpha', []),
  ).rejects.toBeInstanceOf(TypeError);
  await expect(
    credentials.requestMediaKeySystemAccess('com.microsoft.playready', [{}]),
