...
test('repeated completed EME sessions replace stored results and remain deletable as one capture', async () => {
  ...
  try {
    ...
    await context.route('https://replacement.test/**', (route) =>
      route.fulfill({
        contentType: route.request().url().endsWith('.mpd') ? 'application/dash+xml' : 'text/html',
        body: route.request().url().endsWith('.mpd')
          ? `<MPD xmlns="urn:mpeg:dash:schema:mpd:2011" xmlns:cenc="urn:mpeg:cenc:2013"><Period><AdaptationSet><ContentProtection><cenc:pssh>${pssh.toString('base64')}</cenc:pssh></ContentProtection></AdaptationSet></Period></MPD>`
    ...
    await popup.screenshot({
      path: 'output/playwright/session-replacement/dark.png',
      fullPage: true,
    });
