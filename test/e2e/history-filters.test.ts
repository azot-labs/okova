...
test('filters and ordering control visible history and both export formats', async () => {
  ...
  try {
    ...
    try {
      ...
      expect(JSON.parse(await readFile(orderedPath, 'utf8'))).toEqual({
        version: 2,
        captures: [...records].reverse().map((record) => ({
          url: record.url,
          createdAt: record.createdAt,
          playlists: [],
          sessions: [{ pssh: [], records: [record] }],
        })),
      });
      ...
      for (const format of ['JSON', 'TXT']) {
        ...
        if (format === 'JSON')
          expect(JSON.parse(content)).toMatchObject({
            version: 2,
            captures: [{ sessions: [{ records: [records[0]] }] }],
          });
      ...
      for (const format of ['JSON', 'TXT']) {
        const button = popup.getByRole('button', { name: format, exact: true });
